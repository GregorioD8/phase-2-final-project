...
function Stock({ stock, onBuyStock, allTickers, onOrder }) {
  ...
  const stonk = allTickers.find((t) => t.ticker === stock.ticker)


  function handleSubmit(e) {
    e.preventDefault()
    const updatedShares = parseInt(stonk.shares) + input


    //adds stock to portfolio and updates total shares owned
    fetch(` http://localhost:8080/tickers/${stonk.id}`, {
      method: "PATCH",
      headers: {
    ...
    fetch(`https://www.alphavantage.co/query?function=GLOBAL_QUOTE&symbol=${stonk.ticker}&apikey=${APIkey}`)
      .then((res) => res.json())
      .then((d) => {
        console.log(d)
    ...
    <div className="flex flex-wrap justify-center my-4" >
      ...
      <a className="block max-w-lg p-6 bg-white border border-gray-200 rounded-lg shadow hover:bg-gray-100 dark:bg-gray-800 dark:border-gray-700 dark:hover:bg-gray-700">
        ...
        <div className="relative mb-3" >
          ...
          <lable className="bg-green dark:text-gray-400">Open Market order
          </lable>
        </div>
      </a>
    </div>
